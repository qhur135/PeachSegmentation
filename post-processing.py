...
def draw_bboxes(ax, bboxes, color='g', alpha=0.8, thickness=2):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        # sinea - get peach width, height
        width = bbox_int[2]-bbox_int[0]
        print("W : ", width)
        height = bbox_int[3]-bbox_int[1]
        print("H : ", height)
        boxInfoW.append(width)
        boxInfoH.append(height)
...
# draw_labels for display
index_cnt = 0 
ripeness = []
def draw_labels(img, # for ripeness display
                masks, # for ripeness display
                ax,
                labels,
                positions,
                scores=None,
    ...
    for i, (pos, label, mask) in enumerate(zip(positions, labels, masks)):
        # jihyeon - predict peach ripeness
        # rgb to lab
        lab_img = cv2.cvtColor(img,cv2.COLOR_RGB2Lab)
        L_plane, a_plane, b_plane = cv2.split(lab_img) # lab split
        
        # get peach pixel in a_plane
        a_mask = a_plane[mask] # mask - peach segmentation result, a_mask - peach pixel in a_plane
        ripeness.append(int(np.mean(a_mask))) # float -> int
        print(f'index: {i}, mean: {int(np.mean(a_mask))}') # peach pixel average

        label_text = class_names[
            label] if class_names is not None else f'class {label}'
        if scores is not None: # display peach info
            label_text = f'|{scores[i]:.02f}'+ f'|ripe: {ripeness[index_cnt]}' +f'|w : {boxInfoW[index_cnt]}' + f'|h : {boxInfoH[index_cnt]}'
        ...
        ax.text(
            ...
            color=text_color,
            fontsize=15, # label font size # default font_size_mask
...
def imshow_det_bboxes(img,
    ...
    if bboxes is not None:
        ...
        draw_labels(
            img, # for ripeness display
            segms, # for ripeness display
            ax,
            labels[:num_bboxes],
            positions,
            scores=scores,
    ...
    # inhyuk - get peach count and center 
    ...
    # draw_bboxes(ax, centers, colors, alpha=1., thickness=20)
    # print(bboxes)
    print(len(centers))
    x = 0
    y = 0
    ...
    cv2.putText(img, f'COUNT: {len(centers)}', (x + int(w/10) - 10,y + int(h/2) + 20), cv2.FONT_HERSHEY_PLAIN, 5, (255,255,255), 2)
